...
export const loader = async ({ request }) => {
  ...
  const [orders, customers] = await Promise.all([
    queryCached(`${shopDomain}:ordersExplorer:${fromKey}:${toKey}`, DEFAULT_TTL, () =>
      db.order.findMany({
        // Order Explorer shows ALL orders in the range, including £0
        // (staff / replacement / warranty) and POS. The Meta-related
        // tiles elsewhere already filter £0 out, but the raw table
        // should surface them so they're visible to the merchant.
